...
public class Network extends Thread {
  ...
  private static Semaphore inBufferReady;
  private static Semaphore inBufferCount;
  private static Semaphore inBufferMutex;
  private static Semaphore outBufferReady;
  private static Semaphore outBufferCount;
  ...
  Network() {
    ...
    inBufferReady = new Semaphore(0);
    inBufferCount = new Semaphore(maxNbPackets);
    inBufferMutex = new Semaphore(1);
    outBufferReady = new Semaphore(0);
    outBufferCount = new Semaphore(maxNbPackets);
...
//          "\n DEBUG : Network.transferIn() - inComingBuffer status " + getInBufferStatus());
  ...
  public void run() {
    ...
    while (true) {
      ...
      if (clientConnectionStatus.equals("disconnected")
          && serverConnectionStatus.equals("disconnected")) {
        break;
      }
      Network.yield();
    }
